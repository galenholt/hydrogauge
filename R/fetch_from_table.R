...
#'
#' @param request_table data.frame with columns c('portal', 'gauge',
#'   'start_time', 'end_time', 'variable', 'units', 'timeunit', 'statistic',
#'   'datatype')
#' @param clean whether to clean the output to a standard set of columns, or
#'   leave as-returned
#'
...
fetch_from_table <- function(request_table,
                             clean = TRUE) {
  if (!all(c("portal", "gauge", "start_time", "end_time", "variable", "units",
             "timeunit", "statistic", "datatype") %in% names(request_table))) {
    rlang::abort(c("Missing columns in `request_table`",
      "i" = "Requires c('portal', 'gauge', 'start_time', 'end_time', 'variable',
      'units', 'timeunit', 'statistic', 'datatype')"
    ))
  }
...
send_to_type <- function(request_table,
  ...
  # CMD CHECK happy foreach
  r <- NULL

  if (all(request_table$portal_type == "kiwis")) {
    # Still using foreach, though furrr would be better and would let us wrap it
    # all up in a safely. And somehow would be good to manage the
    # parallelisation between here and the inner Hydstra loops. I think here is
    # probably the place to put most of it, since if we're using this function,
    # we only have one variable, etc, and that's what the inner ones loop over.


    ts_out <- foreach::foreach(
      r = 1:nrow(request_table),
      .combine = dplyr::bind_rows,
      .options.future = list(seed = TRUE)
    ) %dofuture% {
      ts <- fetch_kiwis_timeseries(
        portal = request_table$portal[r],
  ...
  if (all(request_table$portal_type == "hydstra")) {
    ...
    if (clean & ncol(ts_out) > 0) {
      ts_out <- ts_out |>
        ...
        dplyr::mutate(
          variable_name = stringr::str_remove_all(.data$variable_short_name, " \\(.*"),
          units = stringr::str_remove_all(.data$variable_short_name, ".*\\(|\\)"),
          source = portal
        ) |>
