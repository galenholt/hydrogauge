...
find_ts_id <- function(portal,
  ...
  if (!is.null(variable)) {
    ...
    ts_list <- ts_list |>
      dplyr::filter(grepl(variable, parametertype_name, ignore.case = TRUE))
  }
  ...
  # We have to pull the datatype, time period, and calculation off out of the name, since I can't find the key to the ts_path
  if (!is.null(datatype)) {
    datatype <- paste0(datatype, collapse = '|')
    ts_list <- ts_list |>
      dplyr::filter(grepl(datatype, ts_name, ignore.case = TRUE))
  }

  if (!is.null(timeunit)) {
    timeunit <- paste0(timeunit, collapse = '|')
    ts_list <- ts_list |>
      dplyr::filter(grepl(timeunit, ts_name, ignore.case = TRUE))
  }

  if (!is.null(statistic)) {
    statistic <- paste0(statistic, collapse = '|')
    ts_list <- ts_list |>
      dplyr::filter(grepl(statistic, ts_name, ignore.case = TRUE))
  }
  ...
  if (!is.null(namefilters)) {
    ...
    ts_list <- ts_list |>
      dplyr::filter(grepl(i, ts_name, ignore.case = TRUE))
  }
