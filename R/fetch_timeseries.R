...
#' Mostly a lot of translating between hydstra and kiwis to allow symmetric
#' specification.
#'
#' @inheritParams fetch_kiwis_timeseries
#'
#' @param datatype as in [fetch_kiwis_timeseries()], or the `datasource`
#'   argument for hydstra, e.g. [fetch_hydstra_timeseries()]
#' @param portal_type I want to either be able to pass this a vector of length
#'   `portal` or 'auto'
#' @param gauge_portal I want to either be able to pass this a named list of
#'   length `portal` or a tibble with `portal` and `gauge` columns, or 'auto' to
...
fetch_timeseries <- function(portal,
                             ...
                             robustness = "robust",
                             check_output = TRUE) {
  # I'm not actually convinced this is that good of a plan. It's a lot of API
  # calls and such to get here. But maybe.
  portal_tib <- build_request_table(
    portal = portal,
    gauge = gauge,
    ...
    if (nrow(all_out) > 0) {
      gaugedoubles <- all_out |>
        dplyr::reframe(
          ...
          which_sources = unique(.data$source),
          .by = "gauge"
        ) |>
        dplyr::filter(.data$n_sources > 1) |>
        dplyr::summarise(
          which_sources = paste0(unique(.data$which_sources), collapse = ", "),
          .by = "gauge"
        ) |>
        dplyr::mutate(gluer = paste(.data$gauge,
                                    .data$which_sources, sep = ": "))
