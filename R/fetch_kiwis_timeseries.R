...
#' = portal, station_no = gauge)` to see the [getTimeseriesList()] output that is
#' parsed to give the `ts_id`. This can be very helpful if you're getting
...
fetch_kiwis_timeseries <- function(portal,
  ...
  # Try to avoid over-call errors that say they're triggered at 250,000 records, but actually seem like 120,000 ish
  ts_ids <- ts_ids |> dplyr::mutate(days = as.numeric(to-from)+1,
                                    sumdays = cumsum(days))

  # This gives a lenght-1 list if the sum is < 120000, otherwise a list of bits each of which is less than 120000
    ts_ids <- ts_ids |>
      dplyr::mutate(chunk = floor(sumdays/120000)) |>
      dplyr::group_by(chunk) |>
      dplyr::group_split()

    timeseries <- foreach::foreach(i = ts_ids,
                          .combine = dplyr::bind_rows,
                          .options.future = list(seed = TRUE)) %dofuture% {
                            getTimeseriesValues(portal = portal,
                                                ts_id = i$ts_id,
    ...
    # Move to this.
    # timeseries <- furrr::future_map(ts_ids, \(x) getTimeseriesValues(portal = portal,
    #                               ts_id = x$ts_id,
    #                               start_time = start_req,
    #                               end_time = end_req,
    #                               period = period,
