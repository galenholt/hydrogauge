...
get_groups <- function(portal,
  ...
  # This list structure is strange, and we can't just call $return because the
  # names are sometimes names and sometimes list items.
  returnid <- which(names(response_body) == "return")
  body_tib <- tibble::as_tibble(response_body[returnid]) |> # the [2] drops the error column
    tidyr::unnest_wider(col = tidyselect::where(is.list)) |> # a `return` list
    tidyr::unnest_longer(col = tidyselect::where(is.list))
