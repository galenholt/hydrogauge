...
find_hydstra_request <- function(portal,
  ...
  if ("all" %in% var_list) {
    rlang::warn("`var_list = 'all'` is *very* dangerous, since it applies the same `statistic` (`data_type` in get_ts_traces), i.e. aggregation function, to all variables, which is rarely appropriate. Check the variables available for your sites and make sure you want to do this.")
  }
  ...
  # Available variables, start and end times, and sites use 'db_default'
  # return_timezone since this passes times around in the state format
  hyd_req_tib <- get_variable_list(
    portal = portal,
    site_list = gauge,
    ...
    dplyr::select(
      "site", "short_name", "variable", "var_name", "datasource",
      "period_start", "period_end"
    ) |>
    dplyr::mutate(varfrom = .data$variable, varto = .data$variable)
  ...
  # If variable is NA, we won't be able to request it, and it's likely because
  # we asked for a gauge that doesn't exist in this portal. I'm tempted to throw
  # a warning, but that will get messy if I rely on this later to toss extra
  # gauges.
  ...
  hyd_req_tib <- hyd_req_tib |>
    dplyr::mutate(
      parametertype_name = stringr::str_remove_all(.data$var_name, " \\(.*"),
      ts_unitsymbol = stringr::str_remove_all(.data$var_name, ".*\\(|\\)")
    )
  ...
  if ("all" %in% var_list) {
    ...
    if (length(var_list) > 1) {
      rlang::warn("var_list has more than one entry but contains 'all'. Using 'all'.")
    }
    ...
    # hyd_req_tib has variables with .00 on the end, var_list might or might
  ...
  if (!is.null(variable)) {
    ...
    hyd_req_tib <- hyd_req_tib |>
      dplyr::filter(grepl(variable_search, .data$parametertype_name, ignore.case = TRUE))
  }
  ...
  if (!is.null(var_list)) {
    ...
    if (ignore_fromderived) {
      ...
      if (length(dfrom) > 0) {
        rlang::inform(c(
          "Derived variables present as var_to",
  ...
  # functions. If we have an explicit var_list (i.e. types of data to pull, we
  # can have length-matched data_type, i.e aggregation function) If var_list ==
  # 'all', we can't match because we don't know the length or order.
  ...
  # Add some constants on (which may gain length at some point, but are not
  # included above because not part of the searching). Including them here
  # though as this produces the full set of info we need for a request
