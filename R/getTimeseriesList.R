...
#' [get_variable_list()] (to a close approximation). The available return fields
#' (and thus factors that can be filtered) are `'station_name', 'station_no',
#' 'station_id', 'ts_id', 'ts_name', 'ts_path', 'parametertype_id',
#' 'parametertype_name'`, where the `station_no` argument matches `station_no`
...
#' most of what is returned by `getStationList` is available, e.g.
#' 'station_latitude'
#'
#'
#' @inheritParams getStationList
#' @param extra_list a named list, see [getStationList()], with a special note
...
#'   the ability to extract it. Thus, including a `timezone` in `extra_list` may
#'   yield unexpected outcomes when requesting dates. A better option is to use
#'   `return_timezone` to adjust the return values. That said, it may be that
#'   some databases return gauge-local tzs, which won't be allowed to be
#'   concatenated. A solution would be to just work in UTC with `timezone =
#'   'UTC'` in extralist to make all outputs on the same tz.
...
#'   uses the API default. BOM defaults to +10. If `'char'` or `'raw'`, returns
#'   the time column as-is from the API (A string in the format
#'   `'YYYY-MM-DDTHH:MM:SS+TZ'`)
#'
#' @return A tibble of information about available timeseries.Times are POSIXct
#'   in UTC by default.
#' @export
#'
getTimeseriesList <- function(portal,
  ...
  # station_no and returnfields need to be a comma separated length-1 vector.
  ...
  api_query_list <- list(
    service = "kisters",
    datasource = 0, # presumably there are others, but this is in documentation.
    type = "queryServices",
  ...
  if (grepl("coverage", returnfields)) {
    ...
    bodytib <- bodytib |>
      dplyr::mutate(
        from = parse_bom_times(.data$from, return_timezone),
        to = parse_bom_times(.data$to, return_timezone),
