...
#'
#' @return typically `bodytib`, unless there are errors, then determined by
#'   `.errorhandling`
#' @keywords internal
#'
ts_error_catch <- function(bodytib, call = rlang::caller_env(),
  ...
  if (.errorhandling == "stop") {
    ermessage <- paste0(
      "API error number ", bodytib$error_num,
