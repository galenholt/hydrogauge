...
#'
#' @examples
#' checkdata <- get_datasources_by_site(portal = "vic",
...
#' @param returntype `'plot'` or `'table'` Plot much more useful- not sure why
#'   the user wouldn't just make a table.
#'
...
plot_datasources_by_site <- function(ds_by_site, returntype = "plot") {
  ...
  if (returntype == "plot") {
    allopts <- ds_by_site |>
      tidyr::expand(.data$site, .data$datasource)

    ds_in_data <- ds_by_site |>
      dplyr::mutate(indata = TRUE) |>
      dplyr::right_join(allopts) |>
      dplyr::mutate(indata = ifelse(is.na(.data$indata), FALSE, .data$indata))

    data_gg <- ggplot2::ggplot(
      ds_in_data,
      ggplot2::aes(
        x = .data$datasource, y = .data$site,
        fill = .data$indata
      )
    ) +
