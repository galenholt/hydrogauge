test_that("simple works", {
  bomout <- getStationList(site_list = c('410730', 'A4260505'))
...
test_that("extra_list", {
  bomout <- getStationList(extra_list = list(station_name = 'River Murray*'))
...
test_that("returnfields and all data", {
  bomout <- getStationList(returnfields = c('station_id', 'station_no', 'station_name', 'site_id', 'station_latitude', 'station_longitude'))

namevec <- c('station_id', 'station_no', 'station_name', 'site_id', 'station_latitude', 'station_longitude')
expect_equal(names(bomout), namevec)
  expect_equal(nrow(bomout), 132204)
})
