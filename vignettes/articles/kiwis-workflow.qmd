...
We can visualise the availability of each variable at each gauge (@fig-vars-duration).
...
The [find_ts_id()] function helps us find desired `ts_id` values. It is a wrapper over [getTimeseriesList()] with regex to select what we want. As such, it's not in the raw API workflow, but is very handy. See [the wrapper vignette](kiwis-wrapper.qmd)
