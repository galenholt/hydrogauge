...
That returns a tall dataframe with both the requested values and some site metadata including the site name, location, etc (@tbl-ts), which the user can then split up or plot how they want (e.g. @fig-ts). There are other options that return lists of dataframes if the user does not want all sites and variables combined-
